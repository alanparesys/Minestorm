...
void MotherShipMovement(GameAssets* assets, Collision* collision)
{
    ...
    if (motherShip[0].position.y > -300)
    {
        ...
        if (spawnDelayTimer >= spawnDelay && currentSpawnIndex < 56)
        {
            if (currentSpawnIndex < maxSpawnPoint)
            {
...
void DrawMotherShip(GameAssets* assets)
{
    ...
    {
        DrawTextureEx(assets->motherShipTexture, (Vector2) { motherShip[0].position.x, motherShip[0].position.y }, 0, 1.0f, WHITE);
    }
}
...
static Vector2D GetValidSpawnPosition(float proposedX, float proposedY)
{
    ...
    float newDistance = MIN_SPAWN_DISTANCE_FROM_PLAYER + 50.0f;

    for (int attempt = 0; attempt < 8; attempt++)
    {
        float testAngle = angle + (attempt * M_PI / 4.0f);
...
void BigBasicEnemySpawn(int i)
{
    ...
    float speed = 0.75f + ((float)GetRandomValue(0, 100) / 100.0f) * 0.5f;
